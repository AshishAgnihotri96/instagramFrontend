...
import axiosInstance from "../../actions/axiosInstanceCall";
...
const Profile = () => {
  ...
  const settingLoggedInUserData=async()=>{
    const { data } = await axiosInstance.get(`${BASE_URL}/api/v1/user/${loggedInUser.username}`);
  ...
  const hostname =
    user && user.website
      ...
      : "No website provided";
  const websiteUrl = user && user.website;
  return (
    <>
      <MetaData
            ...
            {/* posts grid data */}
             {
              ...
              {savedTab ? (
                <PostContainer posts={userdata.saved} id={"saved"} />
              ) : user?.posts?.length > 0 ? (
                <PostContainer posts={userdata.posts} id={"posts"} />
              ) : (
